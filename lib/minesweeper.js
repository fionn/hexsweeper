...
MineSweeper.prototype=Object.create(HexBoard.prototype,{
	...
	onMouseLeftUp:{value:function(ev){
		...
		if(this.mines.indexOf(this.mouse.toString())>=0 && this.selected==this.leftStyle)
			this.game_over=true;
	}},
	...
	drawMineCount:{value:function(context){
		...
		console.log(count,this.unfoundMineCount())
		count--
		while(count>0) {
			if(count<c) { context.strokeStyle='red' }
			if((reduction+offset)%6==5)
				r*=0.8
			context.hexagon(a.x,a.y,r,count,count+1);
			context.stroke()
			reduction++
			count--
		}
	...
	render:{value:function(context){
		...
		for(var i in this.clicked) {
			...
			context.hexagon(a.x,a.y,this.r*this.min()*0.5,3,count+3);
			context.stroke();
		}
		...
		context.hexagon(l.x,l.y,this.r*this.min()*0.8,0,6)
		context.fill()
		if(this.selected==this.leftStyle) {
			context.globalAlpha=0.5
			context.hexagon(l.x,l.y,this.r*this.min(),0,6)
			context.fill()
		...
		context.hexagon(r.x,r.y,this.r*this.min()*0.8,0,6)
		context.fill()
		if(this.selected==this.rightStyle) {
			context.globalAlpha=0.5
			context.hexagon(r.x,r.y,this.r*this.min(),0,6)
			context.fill()
			context.globalAlpha=1
		}
		...
		context.hexagon(m.x,m.y,this.r*this.min()*0.8,0,6)
		context.fill()
		if(this.selected==this.midStyle) {
			context.globalAlpha=0.5
			context.hexagon(m.x,m.y,this.r*this.min(),0,6)
			context.fill()
			context.globalAlpha=1
		}
		...
		if(this.game_over) {
			...
			context.hexagon(a.x,a.y,this.r*this.min(),0,6)
