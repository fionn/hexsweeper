var Hexsweeper=function Hexsweeper(properties){
	...
	this.reset_position=this.mouse_context.r.toRel(0,1.15)
	this.reset_size=0.05
	this.count_position=this.mouse_context.r.toRel(1,-1)
	this.count_size=0.05
...
Hexsweeper.prototype=Object.create(Game.prototype,{
		...
		for(var a=-this.radius;a<=this.radius;++a) {
			for(var b=-this.radius;b<=this.radius;++b) {
				if(this.inBounds({a:a,b:b})){
					...
					var abs=this.context.h.toHex(a,b)
					this.tiles[abs.toString()]={
						a:a,
						b:b,
						button:0,
						isMine:false,
	...
	onMouseMove:{value:function(ev){
		var a=this.mouse_context.a.toAbs(ev.clientX,ev.clientY)
	...
	onMouseUpAfter:{value:function(ev){
		...
		if(d(this.mouse.r,this.reset_position)<2*this.reset_size) {
			return this.reset();
		}
		...
		for(var s in this.selected) {
			if(this.tiles[this.selected[s]].button==2 && this.mines.indexOf(this.selected[s])>=0)
				mine_count++
		}
		if(mine_count==this.mines.length || this.mines.length-mine_count==this.tileCount-this.selected.length) {
			this.game_over='win'
			this.needs_rendering=Object.keys(this.renders)
		}
	}},
	...
	renderSelected:{value:function(){
		...
		// draw counter
		if(this.doRenderUI) {
			var r=this.count_size;
			var r_decrease=r/(Math.ceil(this.mines.length/6)+1)
			var c=0;
			for(var s in this.selected)
				if(this.tiles[this.selected[s]].button==2)
					c++
			this.selected_context.r.lineWidth=r_decrease/10
			var count=this.mines.length;
			this.selected_context.r.strokeStyle='white'
			while(count>0) {
				if(count<=c) { this.selected_context.r.strokeStyle='red' }
				if(count%6==0) r-=r_decrease
				this.selected_context.r.strokeHexagon(this.count_position,r,count,count+1)
				count--
			}
		}
	...
	renderUI:{value:function(){
		...
		if(this.game_over=='win') {
			this.ui_context.r.fillStyle='#FF0';
			this.ui_context.r.shadowColor='#FF0';
			this.ui_context.r.fillHexagon(this.reset_position,this.reset_size);
		} else if(this.game_over=='lose') {
			this.ui_context.r.fillStyle='#FFF';
			this.ui_context.r.fillHexagon(this.reset_position,this.reset_size);
		} else {
			this.ui_context.r.strokeHexagon(this.reset_position,this.reset_size);
		}
	}},
	
	drawSelected:{value:function(context){context.drawImage(this.selected_context.canvas,0,0);}},
	drawGrid    :{value:function(context){if(!this.doRenderGrid) return; context.drawImage(this.grid_context.canvas,0,0);}},
	drawUI      :{value:function(context){if(!this.doRenderUI) return; context.drawImage(this.ui_context.canvas,0,0);}},
	...
	update:{value:function() {
		if(this.mouse_down_time==0)
			this.effective_mouse_down=+this.mouse_down_mod+1+this.mouse_button>>1
		else {
