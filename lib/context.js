var HexContext=function HexContext(context, rescale) {
	this.__lineWidth=0.01;
	this.__shadowBlur=0.01;
	this.context=context;
	this.tileRadius=0.01;
	this.radius=6;
	this.rescale=rescale?rescale:0.9;
}
...
Object.defineProperties(HexContext.prototype,{
	...
	min:{get:function(){return this.rescale*Math.min(this.canvas.width,this.canvas.height)}},
	...
	imageTile :{value:function(h){
		...
		if(typeof(this.__imageHexagon)==='undefined') {
			...
			var ctx=new HexContext(this.__imageHexagon.getContext('2d'),1)
			ctx.tileRadius=0.5
	...
	strokeGrid:{value:function(){
		...
		var ctx=new HexContext(tmp.getContext('2d'),this.rescale)
